...
// AddIgnoreAffectedRow
//
//	@Description: 忽略插入并返回受影响的行
//	@param ctx
//	@param tx
//	@param data
//	@param args
//	@return row
//	@return err
func AddIgnoreAffectedRow(ctx context.Context, tx *gorm.DB, data interface{}, args ...interface{}) (row int64, err error) {
	db := GetDB(ctx, tx)
	argLen := len(args)
	if argLen > 0 {
		if argLen == 1 {
	...
	db = db.Clauses(clause.Insert{Modifier: "IGNORE"}).Create(data)
	row = db.RowsAffected
	err = db.Error
	return
}
